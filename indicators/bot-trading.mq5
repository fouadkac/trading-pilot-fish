...
input double         hedgevolumefactor = 1.0;     // Hedge Volume Factor
...
int OnInit() 
{
   // Définition du magic number avec l'horodatage actuel
   ...
   // Vérification de la date d'expiration de la démo
   if (TimeCurrent() < StringToTime("2026.01.03 00:00")) 
   {
      Print(__FUNCTION__, " >> Demo is still active ... 😊");
   } 
   else 
   {
      Print(__FUNCTION__, " >> Demo is expired... ☹ and happy birthday Fouad 🎂 ");
      ExpertRemove();
      return INIT_FAILED;
   }
